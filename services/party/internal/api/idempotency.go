...
// different payload" and return 409, per party-cif.yaml.
...
// state-changing party-cif.yaml operation declares: a retried call with
// the same key and the same payload replays the original response
// rather than re-running the handler; the same key reused with a
// materially different payload is refused with 409.
//
...
// findOrCreateParty's own dedup engine (SSN / name+DOB exact match)
// makes a second attempt converge on the same existing party rather
// than create a duplicate, and updateParty/tombstoneParty are already
// idempotent at the domain level, and addIdentityDocument is version-
// additive (see internal/service). This is deliberately "at-least-once,
// safe on retry" rather than a strict single-transaction idempotency
// guarantee across the business write and the cache write.
