...
func TestJWKSValidator_ValidToken_ReturnsClaims(t *testing.T) {
	...
	if !claims.HasScope(ScopePartyRead) || !claims.HasScope(ScopePartyWrite) {
...
func TestClaims_HasScope(t *testing.T) {
	c := Claims{Scopes: []string{"party:read"}}
	if !c.HasScope("party:read") {
		t.Fatalf("expected HasScope(party:read) to be true")
	}
	if c.HasScope("party:write") {
		t.Fatalf("expected HasScope(party:write) to be false")
	}
}

func TestTokenClaims_ScopesFallsBackToScpArray(t *testing.T) {
	c := tokenClaims{Scp: []string{"party:read", "party:write"}}
	got := c.scopes()
	if len(got) != 2 || got[0] != "party:read" || got[1] != "party:write" {
