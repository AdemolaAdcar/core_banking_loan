...
func TestJWKSValidator_ValidToken_ReturnsClaims(t *testing.T) {
	...
	tok := signToken(t, key, "kid-1", tokenClaims{
		...
		Scope: "gl:journal-entry:read gl:journal-entry:write",
	})
	...
	if !claims.HasScope(ScopeJournalEntryRead) || !claims.HasScope(ScopeJournalEntryWrite) {
...
func TestClaims_HasScope(t *testing.T) {
	c := Claims{Scopes: []string{"gl:journal-entry:read"}}
	if !c.HasScope("gl:journal-entry:read") {
		t.Fatalf("expected HasScope(gl:journal-entry:read) to be true")
	}
	if c.HasScope("gl:journal-entry:write") {
		t.Fatalf("expected HasScope(gl:journal-entry:write) to be false")
	}
}

func TestTokenClaims_ScopesFallsBackToScpArray(t *testing.T) {
	c := tokenClaims{Scp: []string{"gl:journal-entry:read", "gl:journal-entry:write"}}
	got := c.scopes()
	if len(got) != 2 || got[0] != "gl:journal-entry:read" || got[1] != "gl:journal-entry:write" {
