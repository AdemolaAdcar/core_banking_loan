...
// withScope enforces gl-posting-engine.yaml's serviceAuth security
// scheme: every route requires a valid bearer token AND the specific
// scope that operation declares — there is no unauthenticated route on
// this service, especially not the one permitted to write a journal
// entry.
