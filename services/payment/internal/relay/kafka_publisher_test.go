...
func TestPublishUnpublished_WritesEachEntryToItsOwnTopic_ThenMarksPublished(t *testing.T) {
	entries := []outbox.Entry{
		{ID: "e1", Topic: "payment.disbursement.confirmed", PayloadJSON: []byte(`{"instructionId":"instr-1"}`), CreatedAt: time.Now()},
		{ID: "e2", Topic: "payment.disbursement.failed", PayloadJSON: []byte(`{"instructionId":"instr-1"}`), CreatedAt: time.Now()},
	}
	...
	if writer.written[0].Topic != "payment.disbursement.confirmed" || writer.written[1].Topic != "payment.disbursement.failed" {
...
func TestPublishUnpublished_WriteFails_NothingMarkedPublished(t *testing.T) {
	entries := []outbox.Entry{{ID: "e1", Topic: "payment.disbursement.confirmed", PayloadJSON: []byte(`{}`), CreatedAt: time.Now()}}
	reader := &fakeReader{unpublished: entries}
...
func TestPublishUnpublished_RespectsBatchSize(t *testing.T) {
	...
	for i := range entries {
		entries[i] = outbox.Entry{ID: string(rune('a' + i)), Topic: "payment.disbursement.confirmed", PayloadJSON: []byte(`{}`), CreatedAt: time.Now()}
	}
	reader := &fakeReader{unpublished: entries}
	writer := &fakeWriter{}
