...
func TestPublishUnpublished_WritesEachEntryToItsOwnTopic_ThenMarksPublished(t *testing.T) {
	entries := []outbox.Entry{
		{ID: "e1", Topic: "crm.interaction.logged", PayloadJSON: []byte(`{"partyId":"p1"}`), CreatedAt: time.Now()},
		{ID: "e2", Topic: "crm.case.opened", PayloadJSON: []byte(`{"partyId":"p2"}`), CreatedAt: time.Now()},
	}
	...
	if writer.written[0].Topic != "crm.interaction.logged" || writer.written[1].Topic != "crm.case.opened" {
...
func TestPublishUnpublished_WriteFails_NothingMarkedPublished(t *testing.T) {
	entries := []outbox.Entry{{ID: "e1", Topic: "crm.interaction.logged", PayloadJSON: []byte(`{}`), CreatedAt: time.Now()}}
	reader := &fakeReader{unpublished: entries}
...
func TestPublishUnpublished_RespectsBatchSize(t *testing.T) {
	...
	for i := range entries {
		entries[i] = outbox.Entry{ID: string(rune('a' + i)), Topic: "crm.interaction.logged", PayloadJSON: []byte(`{}`), CreatedAt: time.Now()}
	}
	reader := &fakeReader{unpublished: entries}
	writer := &fakeWriter{}
