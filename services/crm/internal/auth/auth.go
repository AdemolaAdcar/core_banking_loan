...
// crm.yaml's `serviceAuth` security scheme requires on every operation,
// and checks the token carries the scope that operation declares.
// Nothing in internal/api trusts a request until it has passed through
// here.
package auth
...
// Scope names match crm.yaml's securitySchemes.serviceAuth.flows
// exactly — do not add a scope here that isn't also declared there.
const (
	ScopeInteractionWrite     = "crm:interaction:write"
	ScopeCaseWrite            = "crm:case:write"
	ScopeCaseRead             = "crm:case:read"
	ScopeCustomer360Read      = "crm:customer360:read"
	ScopeRelationshipMgrWrite = "crm:relationshipmanager:write"
	ScopeRelationshipMgrRead  = "crm:relationshipmanager:read"
	ScopeCommPrefsWrite       = "crm:commprefs:write"
	ScopeCommPrefsRead        = "crm:commprefs:read"
)
