...
// consumer of loan.account.booked/loan.account.disbursed/etc elsewhere
// in this system is already expected to be idempotent (same assumption
...
// message (entries for every loan.*/delinquency.*/payment.* event type
// share one Writer), and kafka-go rejects a Writer that has both a fixed
// Topic and per-message topics set.
