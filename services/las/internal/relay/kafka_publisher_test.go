...
func TestPublishUnpublished_WritesEachEntryToItsOwnTopic_ThenMarksPublished(t *testing.T) {
	entries := []outbox.Entry{
		{ID: "e1", Topic: "loan.account.booked", PayloadJSON: []byte(`{"loanAccountId":"la1"}`), CreatedAt: time.Now()},
		{ID: "e2", Topic: "loan.account.disbursed", PayloadJSON: []byte(`{"loanAccountId":"la1"}`), CreatedAt: time.Now()},
	}
	...
	if writer.written[0].Topic != "loan.account.booked" || writer.written[1].Topic != "loan.account.disbursed" {
...
func TestPublishUnpublished_WriteFails_NothingMarkedPublished(t *testing.T) {
	entries := []outbox.Entry{{ID: "e1", Topic: "loan.account.booked", PayloadJSON: []byte(`{}`), CreatedAt: time.Now()}}
	reader := &fakeReader{unpublished: entries}
...
func TestPublishUnpublished_RespectsBatchSize(t *testing.T) {
	...
	for i := range entries {
		entries[i] = outbox.Entry{ID: string(rune('a' + i)), Topic: "loan.account.booked", PayloadJSON: []byte(`{}`), CreatedAt: time.Now()}
	}
	reader := &fakeReader{unpublished: entries}
	writer := &fakeWriter{}
